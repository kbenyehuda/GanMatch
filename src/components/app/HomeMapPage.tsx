...
export interface HomeMapPageProps {
  /** When set (e.g. `/gan/[id]`), map focuses this gan and it stays in the dataset while panning. */
  seedGan?: Gan | null;
}

export function HomeMapPage({ seedGan = null }: HomeMapPageProps) {
  ...
  const preserveGanIds = useMemo(() => {
    ...
    if (selectedGan?.id) s.add(selectedGan.id);
    if (seedGan?.id) s.add(seedGan.id);
    return s.size > 0 ? s : undefined;
  }, [selectedGan?.id, seedGan?.id]);
  ...
  useEffect(() => {
    if (!seedGan || seedFocusedRef.current) return;
    seedFocusedRef.current = true;
    setFitToAddress({ lon: seedGan.lon, lat: seedGan.lat, zoom: 16 });
    const t = window.setTimeout(() => setFitToAddress(null), 850);
  ...
  return (
    <div className="relative w-full h-screen min-h-[100dvh] overflow-hidden" dir="rtl">
      ...
      <div className="absolute top-[max(1rem,env(safe-area-inset-top))] left-4 right-4 md:right-[calc(24rem+1rem)] z-10 flex flex-wrap items-center justify-between gap-y-2">
        <Link
          href="/"
          className="flex items-center gap-2 bg-white/95 backdrop-blur px-4 py-2 rounded-full shadow-lg hover:bg-white transition-colors"
          title="מפת גנים"
        >
