...
export function applyFilters(ganim: Gan[], filters: GanFilters): Gan[] {
  return ganim.filter((g) => {
    if (filters.friday_schedule != null && g.friday_schedule !== filters.friday_schedule) return false;
    if (filters.meal_type != null && g.meal_type !== filters.meal_type) return false;
    ...
    if (filters.kosher_status != null && g.kosher_status !== filters.kosher_status) return false;
    ...
    if (filters.vacancy_status != null && g.vacancy_status !== filters.vacancy_status) return false;
