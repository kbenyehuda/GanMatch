-- Single-gan fetch for shareable /gan/[id] URLs (same row shape as get_ganim_in_bbox).
--
-- Not found: WHERE g.id = p_id matches no row → RETURNS zero rows. No error/exception.
-- Callers treat empty array as 404 / null.

CREATE OR REPLACE FUNCTION public.get_gan_by_id(p_id UUID)
RETURNS TABLE (
...
GRANT EXECUTE ON FUNCTION public.get_gan_by_id(UUID) TO anon;
GRANT EXECUTE ON FUNCTION public.get_gan_by_id(UUID) TO authenticated;
